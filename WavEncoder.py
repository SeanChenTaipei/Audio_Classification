...
def get_dataset(path: str, df_MH: pd.DataFrame) -> Dataset:
    ...
    for index, name in enumerate(tqdm(id_list)):
        ...
        data["file"] = file
        data["audio"] = dict(
            path=file, array=torch.as_tensor(sample), sampling_rate=16000
        )
...
if __name__ == "__main__":
    ...
    for index, (wav_path, csv_path) in enumerate(
        zip(
            [args.train_wav, args.public_wav, args.private_wav],
        ...
        dset = d.map(
            lambda data: preprocess_function(
                data, feature_extractor, t_mapping.get(index)
            ),
        ...
        for i, data in tqdm(enumerate(dset)):
            ...
            if mapping.get(index) == "train":
                inputs = processor(
                    d[i]["audio"]["array"], sampling_rate=16000, return_tensors="pt"
                )
            else:
                inputs = processor(
                    dset[i]["input_values"], sampling_rate=16000, return_tensors="pt"
                )
    ...
    for index, (wav_path, csv_path) in enumerate(
        ...
        dset = d.map(
            lambda data: preprocess_function(data, feature_extractor),
            remove_columns="audio",
            batched=True,
        )
        # Audio file is decoded on the fly
