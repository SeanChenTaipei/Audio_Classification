        ...
        for model in self.models:
            ...
            if BBC in algo:
                predictor = BalancedBaggingClassifier(
                    estimator=predictor, **model["bbc_params"]
                )
            pipeline_dict[algo] = predictor

        weights_dict = (
            {model["algo"]: model["weight"] for model in self.models}
            if "weight" in self.models[0].keys()
            else {p: 1 / len(pipeline_dict) for p in pipeline_dict.keys()}
        )
